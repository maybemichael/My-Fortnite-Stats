...
struct SquadStats: Codable {
    ...
    enum SquadStatsKeys: String, CodingKey {
        case wins = "top1"
        case kills
        case winRatio
    ...
    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: SquadStatsKeys.self)
        ...
        let kdContainer = try container.nestedContainer(keyedBy: SquadStatsKeys.DisplayKeys.self, forKey: .killDeathRatio)
        killDeathRatio = try kdContainer.decode(String.self, forKey: .displayValue)
        let top1Container = try container.nestedContainer(keyedBy: SquadStatsKeys.DisplayKeys.self, forKey: .wins)
        wins = try top1Container.decode(String.self, forKey: .displayValue)
        let killsContainer = try container.nestedContainer(keyedBy: SquadStatsKeys.DisplayKeys.self, forKey: .kills)
        kills = try killsContainer.decode(String.self, forKey: .displayValue)
        let winRatioContainer = try container.nestedContainer(keyedBy: SquadStatsKeys.DisplayKeys.self, forKey: .winRatio)
        winRatio = try winRatioContainer.decode(String.self, forKey: .displayValue)
        let matchesContainer = try container.nestedContainer(keyedBy: SquadStatsKeys.DisplayKeys.self, forKey: .matches)
        matches = try matchesContainer.decode(String.self, forKey: .displayValue)
        let kpgContainer = try container.nestedContainer(keyedBy: SquadStatsKeys.DisplayKeys.self, forKey: .killsPerGame)
